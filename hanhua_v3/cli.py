...
def create_checkpoint() -> str:
    ...
    try:
        ...
        return git_command(
            "rev-parse", "--short", "HEAD", capture=True
...
def read_git_queue_rows(ref: str) -> list[dict[str, str]]:
    try:
        payload = git_command(
            "show", f"{ref}:data/new_translations.tsv", capture=True
...
def run_scan(args: argparse.Namespace) -> int:
    ...
    scanner_args = [
        ...
        str(DEFAULT_SCAN),
        "--min-confidence",
        getattr(args, "min_confidence", "medium"),
    ]
    full_text_scanner.main(scanner_args)
    translation_queue.main([
        str(DEFAULT_SCAN),
        "--legacy",
        str(ROOT / "data" / "translations.tsv"),
        "--daily",
        str(args.queue),
        "--output",
...
def run_translate(
    ...
    if args.new_since:
        only_keys = queue_keys_from_rows(read_queue_rows(args.queue)) - queue_keys_from_rows(
            read_git_queue_rows(args.new_since)
        )
...
def run_release(args: argparse.Namespace) -> int:
    ...
    for variant, label in (("simplified", "簡體中文"), ("taiwan", "台灣繁體中文")):
        ...
        try:
            result = run_build(build_args)
        except Exception as exc:
            raise CliError(f"{label} 構建/驗證失敗，停止發布閘門：{exc}") from exc
        if result != 0:
            raise CliError(f"{label} 構建/驗證失敗（exit={result}），停止發布閘門")
        print(f"[Release Gate] {label} PASS")

    print("\n[Release Gate] PASS：兩個獨立產品均通過構建與驗證。")
    print("[Release Gate] 現在才允許將兩個語言包交付給使用者。")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command, func in commands:
        ...
        parser_for_command.set_defaults(func=func)

        if command in {"build", "release"}:
            parser_for_command.add_argument(
                "--source-dir", type=Path, default=ROOT / "src_file" / "DBOZero"
            )
        elif command == "config":
            parser_for_command.add_argument("--game-dir", type=Path)
            parser_for_command.add_argument("--show", action="store_true")
        else:
            add_source_args(parser_for_command)

        if command in {"scan", "translate", "write", "build", "update", "recover", "status"}:
            add_queue_args(parser_for_command)
        if command in {"write", "update"}:
            parser_for_command.add_argument(
                "--output-dir", type=Path, default=ROOT / "output_taiwan"
            )
        ...
        if command == "translate":
            add_translate_args(parser_for_command)
        if command == "update":
            add_translate_args(parser_for_command)
            parser_for_command.add_argument("--translate-all", action="store_true")
            parser_for_command.add_argument("--recover-refs", nargs="*", default=[])
        if command == "recover":
            parser_for_command.add_argument("refs", nargs="+", help="Git refs to recover from")
            parser_for_command.add_argument("--dry-run", action="store_true")
