    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle, delimiter="\t"):
            ...
            rows.append(
                QueueRow(
                    file_name,
                    (row.get("id") or row.get("ID") or row.get("位置") or "").strip(),
                    source,
                    translation,
                    (row.get("encoding") or "").strip().lower(),
                    (row.get("kind") or "").strip().lower(),
                )
            )
    return rows
...
def _patch_text_rows(data: bytes, rows: list[QueueRow], file_name: str) -> tuple[bytes, int]:
    ...
    for row in rows:
        try:
            ...
            raise WriteError(
                f"Non-offset locator cannot patch generic resource: {row.locator!r}"
            ) from exc
        if offset < 0 or offset >= len(data):
            raise WriteError(f"Offset outside resource: 0x{offset:X}")
        ...
        if is_dat:
            ...
            source_single = row.source.replace("\\", "\\\\").replace("'", "\\'")
...
def _detect_lang0_encoding(data: bytes) -> str:
    """Use the file's actual text encoding instead of assuming GBK."""
...
def _write_one(
    ...
    if name == "lang0.pak":
        ...
        encoding = _detect_lang0_encoding(original)
        patched, stats = lang0.patch_lang0_bytes(original, keyed, encoding=encoding)
        changed = stats["changed"]
    elif name in TBL_FILES:
        patched, stats = tbl.patch_tbl_bytes(
            original, [_tbl_override(row, name) for row in rows]
        )
        changed = stats["changed"]
        if stats["missing"]:
            raise WriteError(
...
def _mirror_source_tree(source_root: Path, output_root: Path) -> int:
    """Create the build tree from untouched source, then patch supported files."""
    source_root = source_root.resolve()
    output_root = output_root.resolve()
    if not source_root.is_dir():
        raise WriteError(f"Source root not found: {source_root}")
...
def write_queue(queue: Path, source_root: Path, output_root: Path) -> dict[str, int]:
    ...
    if any(row.file.lower().endswith(".bin") for row in rows):
    ...
    for key, file_rows in sorted(grouped.items()):
        ...
        output, changed = _write_one(path, file_rows, output_root, source_root)
        result[output.relative_to(output_root).as_posix()] = changed
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        print(
            f"translated rows: {len(read_queue(args.queue))}\n"
            "dry-run: no resource files were written"
        )
        return 0
