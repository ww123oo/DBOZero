...
驗證範圍以程式、資源格式、輸出結構與建置結果為準，不包含遊戲內測試要求。

## 8. 目前限制

GitHub CI 不包含使用者本機的完整 DBO 遊戲資源，因此 CI 綠燈代表程式碼與測試通過，不等同於已使用最新遊戲資源產生正式發布包。

拿到官方新版本完整原始遊戲目錄後，應在具備資源的環境執行 `dboc release`，確認兩個語言產品的 build 與 validation 都 PASS，之後即可依專案發布流程處理輸出內容。
