#!/usr/bin/env python3
"""Fix Item Ascend UI, rarity wording, and CC status labels.

- 升階 -> 進階
- 稀有度 -> 稀少度
- Materials / Success Rate / Zeni
- CC: 麻痺 恐怖 混亂 石化 糖果 出血 腹痛 燃燒
"""
...
# Fix main to use BY_ID/BY_EN properly - rewrite clean main below
...
# The above main for new_translations has bugs - provide clean single main:

def main() -> int:
    n1 = fix_translations_tsv(root / "data" / "translations.tsv")
    n2 = fix_new_translations_clean(root / "data" / "new_translations.tsv")
    print(f"OK: translations.tsv ~{n1}, new_translations.tsv ~{n2}")
    return 0


def fix_new_translations_clean(path: Path) -> int:
    if not path.exists():
        return 0
    BY_ID = {
        "DST_ITEMASCEND_TITLE": "裝備進階",
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as f:
        ...
        w.writeheader()
        w.writerows(rows)
    print(f"OK: fixed {n} rows in new_translations")
    return n
...
def main() -> int:
    n1 = fix_translations_tsv(root / "data" / "translations.tsv")
    n2 = fix_new_translations_clean(root / "data" / "new_translations.tsv")
    print(f"OK: translations.tsv ~{n1}, new_translations ~{n2}")
    return 0
